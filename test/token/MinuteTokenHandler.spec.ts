...
import MinuteTokenHandler from "../../src/token/MinuteTokenHandler";

describe("MinuteTokenHandler", () => {
  it("should not complete with 2 as input using m format", () => {
    let handler:MinuteTokenHandler = new MinuteTokenHandler("m", moment.utc(0));
  ...
  it("should accept 00 as input using m format", () => {
    let handler:MinuteTokenHandler = new MinuteTokenHandler("m", moment.utc(0));
    handler.handleInput("0");
    expect(handler.isCompleted).to.be.equal(false);
  ...
  it("should not complete with 2 as input using mm format", () => {
    let handler:MinuteTokenHandler = new MinuteTokenHandler("mm", moment.utc(0));
    handler.handleInput("2");
    expect(handler.isCompleted).to.be.equal(false);
    expect(handler.value).to.be.equal("2");
  });
  ...
  it("should accept 00 as input using mm format", () => {
    let handler:MinuteTokenHandler = new MinuteTokenHandler("mm", moment.utc(0));
    handler.handleInput("0");
    expect(handler.isCompleted).to.be.equal(false);
    expect(handler.value).to.be.equal("0");
